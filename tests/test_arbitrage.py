...
async def test_cex_cex_arbitrage():
    ...
    # Кэшируем данные для Binance
    result = await data_processor.cache_orderbook(
        exchange_id="binance",
        symbol="BTC/USDT",
        orderbook_data={
            "ask": 50000,
    ...
    assert result is True, "Ошибка кэширования для Binance"

    # Кэшируем данные для Bybit
    result = await data_processor.cache_orderbook(
        exchange_id="bybit",
        symbol="BTC/USDT",
        orderbook_data={
    ...
    assert result is True, "Ошибка кэширования для Bybit"

    finder = ArbitrageFinder()
    opportunities = await finder.find_cex_cex_opportunities()

    assert len(opportunities) > 0, f"Найдено {len(opportunities)} возможностей, ожидалось > 0"
    opp = opportunities[0]
...
async def test_cex_cex_cex_arbitrage():
    ...
    assert result is True, "Ошибка кэширования для Binance BTC/USDT"
    ...
    assert result is True, "Ошибка кэширования для Bybit ETH/BTC"

    # Кэшируем данные для MEXC
    result = await data_processor.cache_orderbook(
        exchange_id="mexc",
        symbol="ETH/USDT",
        orderbook_data={"ask": 2500, "askVolume": 1, "bid": 2600, "bidVolume": 1}
    )
    assert result is True, "Ошибка кэширования для MEXC ETH/USDT"
...
async def test_api_cex_cex_endpoint():
    ...
    assert len(data) > 0, f"Найдено {len(data)} возможностей, ожидалось > 0"
